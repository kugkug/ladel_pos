...
export async function fetchPaidProjects() {
  try {
    ...
    const paidProjects = (projects || []).filter(p => {
      ...
      const totalInv = p.invoices?.reduce((sum, inv) => sum + (Number(inv.total_amount) || 0), 0) || 0;
...
export async function fetchPaidCalendarEvents() {
  ...
  return paidProjects.map(p => {
    ...
    if (p.payments && p.payments.length > 0) {
      ...
      const sortedPayments = [...p.payments].sort((a, b) => new Date(b.payment_date) - new Date(a.payment_date));
...
export async function fetchAllCalendarEvents() {
  try {
    ...
    (invoices || []).forEach(inv => {
      ...
      if (paidEvents.some(pe => pe.projectData?.id === inv.project_id)) return;
...
export async function fetchReminders(type = 'all') {
  try {
    let query = supabase.from('reminders').select('*').order('date', { ascending: true });
