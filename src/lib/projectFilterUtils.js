...
export const getCompanyName = (project) => {
  ...
  return project.companyName || project.company_name || project.client_name || project.client || project.company || 'Unknown';
};

export const getDateField = (project) => {
  if (!project) return null;
  return project.created_at || project.date_created || project.createdOn || null;
};
...
export const matchesSearch = (project, searchTerm) => {
  if (!searchTerm || !searchTerm.trim()) return true;
  const term = searchTerm.toLowerCase().trim();
  const searchableFields = getSearchableFields(project);
...
export const applyAllFilters = (projects, filters) => {
  ...
  return projects.filter(project => {
    ...
    if ((start || end) && !matchesDateRange(project, start, end)) return false;
