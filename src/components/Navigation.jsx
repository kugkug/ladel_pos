...
const Navigation = () => {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div
                className={`fixed inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 w-72 bg-[#1B4D5C] text-white transition-transform duration-300 ease-in-out z-50 flex flex-col h-screen shadow-2xl lg:shadow-none`}
            >
                ...
                <nav className='flex-1 px-4 space-y-1.5 overflow-y-auto custom-scrollbar pb-6'>
                    {links.map((link) => {
                        ...
                        const isActive =
                            ...
                            (link.path === '/expenses' &&
                                location.pathname === '/expenses/dashboard') ||
                            (link.path === '/sales' &&
                        ...
                        return (
                            ...
                            >
                                <Icon
                                    className={`w-5 h-5 transition-colors ${isActive ? 'text-white' : 'text-white/70 group-hover:text-white'}`}
                                />
                                {link.name}
                            </Link>
                        );
