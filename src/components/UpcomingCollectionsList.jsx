...
const UpcomingCollectionsList = ({ data, isLoading, error }) => {
  ...
  if (error) {
    return (
      <div className="p-6 bg-red-50 text-red-600 rounded-xl border border-red-200">
        ...
        <p className="font-semibold">Failed to load upcoming collections</p>
      ...
      <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-emerald-50/30">
        <div>
          <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
            <Clock className="w-5 h-5 text-emerald-500" /> Upcoming Collections
          </h3>
          <p className="text-sm text-gray-500 mt-1">Payments due within the next 7 days</p>
        </div>
        {data?.length > 10 && (
        ...
        <Table>
          <TableHeader className="bg-gray-50/80">
            <TableRow>
              ...
              <TableHead className="font-semibold text-gray-700">Days Until Due</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              displayData.map((item) => (
                <TableRow key={item.id} className="hover:bg-emerald-50/40 transition-colors">
                  ...
                  <TableCell>
                    <span className="badge-pending bg-emerald-100 text-emerald-800 border-emerald-200">
                      {item.daysUntilDue === 0 ? 'Today' : `In ${item.daysUntilDue} Days`}
                    </span>
                  </TableCell>
                  <TableCell>
...
export default UpcomingCollectionsList;