...
const QuotationEditModal = ({ isOpen, onClose, data, onSaveSuccess, onPromptPO }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentUser) logUpdate(currentUser, 'SALES', 'QUOTATION', data.quotation_number, data, payload);
      ...
      if (data.quotation_status !== 'Confirmed' && formData.quotation_status === 'Confirmed' && onPromptPO) {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[min(900px,96vw)] max-w-none h-[90vh] flex flex-col bg-white p-0 overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form id="editQuotationForm" onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="px-6 py-4 border-t sticky bottom-0 bg-white z-10 flex justify-end gap-3">
          ...
          <Button type="submit" form="editQuotationForm" disabled={isSubmitting} className="bg-blue-600 text-white min-h-[44px] px-6">
