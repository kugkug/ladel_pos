...
const DREditModal = ({ isOpen, onClose, data, project, onSaveSuccess }) => {
  const { toast } = useToast();
  const [showPreview, setShowPreview] = useState(false);
  const [formData, setFormData] = useState({ dr_number: '', date_delivered: '', dr_status: 'Partial', notes: '' });

  useEffect(() => {
    if (data && isOpen) setFormData({ dr_number: data.dr_number || '', date_delivered: data.date_delivered || '', dr_status: data.dr_status || 'Partial', notes: data.notes || '' });
  }, [data, isOpen]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from('delivery_receipts').update(formData).eq('id', data.id);
      ...
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="max-w-md bg-white p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><Label>DR Number</Label><Input required value={formData.dr_number} onChange={e => setFormData({...formData, dr_number: e.target.value})} className="mt-1" /></div>
            <div><Label>Date Delivered</Label><Input type="date" required value={formData.date_delivered} onChange={e => setFormData({...formData, date_delivered: e.target.value})} className="mt-1" /></div>
            <div>
