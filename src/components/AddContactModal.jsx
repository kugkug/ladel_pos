...
const AddContactModal = ({ companyId, isOpen, onClose, onSaveSuccess }) => {
  const { addContact } = useContext(CompanyContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ contact_name: '', contact_email: '', contact_phone: '', role_title: '', is_primary: false });
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-start space-x-2 pt-2 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
            ...
            <div className="grid gap-1.5 leading-none">
              <label htmlFor="is_primary_add" className="text-sm font-bold text-blue-900 cursor-pointer">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-6">
            ...
            <Button type="submit" disabled={isSubmitting} className="bg-blue-600 hover:bg-blue-700 text-white min-w-[120px]">
              {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null} Save Contact
            </Button>
          </div>
        </form>
      </DialogContent>
