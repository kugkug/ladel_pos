...
import { 
  ...
  getYear, subYears, addYears
} from 'date-fns';
import { ChevronLeft, ChevronRight, Plus, Calendar, X, Trash2, Loader2 } from 'lucide-react';
import { 
...
const CalendarReminders = () => {
  ...
  const [newReminder, setNewReminder] = useState({ date: format(new Date(), 'yyyy-MM-dd'), description: '' });
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Filter Counts
  const [filterCounts, setFilterCounts] = useState({ all: 0, paid: 0, overdue: 0, upcoming: 0 });
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      setFilterCounts({
        ...
        upcoming: allCombined.filter(e => e.type === 'upcoming').length,
      });
    } catch (error) {
  ...
  const handleAddReminder = async (e) => {
    ...
    try {
      ...
      setNewReminder({ date: format(new Date(), 'yyyy-MM-dd'), description: '' });
  ...
  const handleCalendarPrev = () => {
    setCurrentMonth(calendarView === 'year' ? subYears(currentMonth, 1) : subMonths(currentMonth, 1));
  };

  const handleCalendarNext = () => {
    setCurrentMonth(calendarView === 'year' ? addYears(currentMonth, 1) : addMonths(currentMonth, 1));
  };
    ...
    <div className="calendar-reminders-container">
      ...
      <div className="calendar-section">
        <div className="filters-bar">
          ...
          >
            Paid <span className="filter-count">{filterCounts.paid}</span>
          </button>
          <button 
            className={`filter-btn ${activeFilter === 'overdue' ? 'active' : ''}`}
            onClick={() => setActiveFilter('overdue')}
          >
            Overdue <span className="filter-count">{filterCounts.overdue}</span>
          </button>
          <button 
            className={`filter-btn ${activeFilter === 'upcoming' ? 'active' : ''}`}
            onClick={() => setActiveFilter('upcoming')}
          >
            Upcoming <span className="filter-count">{filterCounts.upcoming}</span>
          </button>
        </div>
        ...
        <div className="calendar-header calendar-header-with-view">
          ...
          <div className="calendar-header-nav">
            <button type="button" className="calendar-nav-btn" onClick={handleCalendarPrev} aria-label={calendarView === 'year' ? 'Previous year' : 'Previous month'}>
              <ChevronLeft size={20} />
            </button>
            <h2 className="calendar-month-title">
              {calendarView === 'year' ? String(displayYear) : format(currentMonth, 'MMMM yyyy')}
            </h2>
            <button type="button" className="calendar-nav-btn" onClick={handleCalendarNext} aria-label={calendarView === 'year' ? 'Next year' : 'Next month'}>
            ...
            <div className="calendar-days">
              {isLoadingEvents ? (
                ...
                days.map(day => {
                  ...
                  return (
                    <div key={day.toString()} className={`calendar-day-cell ${!isCurrentMonth ? 'other-month' : ''}`}>
                      <span className={`day-number ${isToday ? 'today' : ''}`}>
                        {format(day, 'd')}
                      </span>
                      <div>
                        {dayEvents.map((evt, idx) => (
                          <div key={idx} className={`event-badge event-${evt.type}`} title={evt.title}>
                            {evt.title}
                          </div>
                        ))}
                      </div>
          ...
          <div className="calendar-year-view">
            {isLoadingEvents ? (
              ...
              Array.from({ length: 12 }, (_, monthIndex) => {
                const monthDays = getDaysForMonth(displayYear, monthIndex);
                const monthStartMini = startOfMonth(new Date(displayYear, monthIndex, 1));
                return (
                  <div key={monthIndex} className="calendar-year-month">
                    <div className="calendar-year-month-title">{format(monthStartMini, 'MMMM')}</div>
                    <div className="calendar-year-weekdays">
                      {weekDaysShort.map((d, i) => (
                        <div key={`${monthIndex}-h-${i}`} className="calendar-year-weekday">{d}</div>
                      ))}
                    </div>
                    <div className="calendar-year-days">
                      {monthDays.map(day => {
                        const inMonth = isSameMonth(day, monthStartMini);
                        const isToday = isSameDay(day, new Date());
                        const dayEvents = getFilteredEventsForDay(day);
                        const showBadges = Math.min(dayEvents.length, 2);
                        const more = dayEvents.length - showBadges;
                        ...
                        return (
                          ...
                          >
                            <span className={`calendar-year-day-number ${isToday ? 'today' : ''}`}>
                              {format(day, 'd')}
                            </span>
                            {inMonth && dayEvents.length > 0 && (
                              <div className="calendar-year-events">
                                {dayEvents.slice(0, showBadges).map((evt, idx) => (
                                  <div key={idx} className={`event-badge event-${evt.type} calendar-year-event-badge`} title={evt.title}>
                                    {evt.title}
                                  </div>
                                ))}
                                {more > 0 && (
                                  <span className="calendar-year-more" title={`${more} more`}>+{more}</span>
                                )}
                              </div>
                            )}
                          </div>
      ...
      <div className="reminders-section">
        <div className="reminders-header flex justify-between items-center">
          <h3 className="reminders-title">
            <Calendar size={20} className="text-blue-600" /> Reminders
          </h3>
          <button 
        ...
        <div className="filters-bar justify-center border-b-0 pb-2">
          ...
          <button 
            className={`filter-btn text-xs py-1 px-3 ${reminderFilter === 'system' ? 'active' : ''}`}
            onClick={() => setReminderFilter('system')}
          >
            System
          </button>
          <button 
            className={`filter-btn text-xs py-1 px-3 ${reminderFilter === 'custom' ? 'active' : ''}`}
            onClick={() => setReminderFilter('custom')}
          >
            Custom
          </button>
        </div>
        ...
        {showAddForm && (
          <form className="add-reminder-form" onSubmit={handleAddReminder}>
            ...
            <div className="form-actions mt-2">
              <button type="button" className="btn-save bg-gray-200 text-gray-800 hover:bg-gray-300" onClick={() => setShowAddForm(false)}>
                Cancel
              </button>
              <button type="submit" className="btn-save" disabled={isSubmitting || !newReminder.description.trim()}>
        ...
        <div className="reminders-list-container">
          {isLoadingReminders ? (
            ...
            reminders.map(rem => (
              <div key={rem.id} className={`reminder-item ${rem.reminder_type}`}>
                <div className="reminder-content">
                  <span className="reminder-date">{format(new Date(rem.date), 'MMM dd, yyyy')}</span>
                  <p className="reminder-desc">{rem.description}</p>
                  <span className={`reminder-badge ${rem.reminder_type}`}>
                    {rem.reminder_type === 'system' ? '⚙️ System' : '✏️ Custom'}
                  </span>
                </div>
                {rem.reminder_type === 'custom' && (
                  <button className="btn-delete" onClick={() => handleDeleteReminder(rem.id)} title="Delete Reminder">
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
