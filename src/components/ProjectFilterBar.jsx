...
const SEARCH_FIELD_OPTIONS = [
    ...
    { value: 'project_number', label: 'PR No.' },
    { value: 'quotation_number', label: 'QT' },
    { value: 'purchase_order', label: 'PO' },
    { value: 'invoice_number', label: 'INV' },
    { value: 'dr_number', label: 'DR' }
];
...
const ProjectFilterBar = ({ filters, setFilters, projects, isLoading }) => {
    ...
    const removeFilter = (filterType) => {
        if (filterType === 'search')
            setFilters({ ...filters, search: '', searchField: 'all' });
    ...
    if (filters.datePreset) dateFilterLabel = filters.datePreset;
        ...
        if (filters.dateRange.start && filters.dateRange.end)
            dateFilterLabel = `${filters.dateRange.start} to ${filters.dateRange.end}`;
        else if (filters.dateRange.start)
            dateFilterLabel = `From ${filters.dateRange.start}`;
        else if (filters.dateRange.end)
            dateFilterLabel = `Until ${filters.dateRange.end}`;
    }
            ...
            <div className='flex flex-col md:flex-row gap-3'>
                ...
                <div className='flex flex-col sm:flex-row gap-2 flex-grow'>
                    ...
                    <div className='relative flex-grow'>
                        ...
                        <Input
                            type='text'
                            placeholder='Search PR / QT / PO / INV / DR'
                ...
                >
                    <PopoverTrigger asChild>
                        <Button
                            variant='outline'
                            className={`h-10 w-full md:w-56 justify-start text-left font-normal bg-gray-50 ${filters.datePreset || filters.customDateActive ? 'border-blue-500 text-blue-700 bg-blue-50' : 'text-gray-700'}`}
                            disabled={isLoading}
                        >
                    ...
                    {filters.search && (
                        <span className='inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200'>
                            ...
                            {SEARCH_FIELD_OPTIONS.find(
                                (o) =>
                                    o.value === (filters.searchField || 'all')
                            )?.label || 'All Columns'}
                            ): "{filters.search}"
                            <button
                                onClick={() => removeFilter('search')}
                    ...
                    {filters.company !== 'All' && (
                        <span className='inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 border border-purple-200'>
                            ...
                            <button
                                ...
                                className='ml-1.5 hover:text-purple-900 focus:outline-none'
                            >
                                <X className='w-3.5 h-3.5' />
                            </button>
                        </span>
