...
export const ProjectProvider = ({ children }) => {
    ...
    const fetchProjects = useCallback(async () => {
        ...
        try {
            const { data, error } = await supabase
                .from('projects')
                .select(
                    `*, company:companies(*), contact:company_contacts(*), quotations!projects_quotation_id_fkey(*), invoice:invoices(*)`
                )
            ...
            const orphaned = data.filter(
                (p) => p.quotation_id && !p.quotations
            );
            if (orphaned.length > 0) {
                console.warn(
                    'Data Integrity Warning: Projects with orphaned quotation_id:',
                    orphaned.map((p) => p.project_number)
                );
            }
    ...
    const createProject = async (projectData, quotationData) => {
        try {
            ...
            const safeCompanyId =
                projectData.company_id || projectData.companyId;
            const safeContactId =
                projectData.contact_id || projectData.contactId;
            const prNumber =
                projectData.pr_number ||
                projectData.prNumber ||
                projectData.prNo ||
                projectData.pr_id ||
                projectData.projectNumber ||
                projectData.project_number;
            const projectTitle =
                projectData.project_title || projectData.projectTitle;

            if (!safeCompanyId || !uuidRegex.test(safeCompanyId))
                throw new Error(
                    'Company is required and must be a valid UUID.'
                );
            if (!safeContactId || !uuidRegex.test(safeContactId))
                throw new Error(
                    'Contact is required and must be a valid UUID.'
                );
            ...
            if (existingProject)
                throw new Error(
                    `PR number "${prNumber.trim()}" already exists in the database.`
                );
            ...
            if (quotationData && quotationData.quotationNumber?.trim()) {
                const qtPayload = {
                    ...
                    quotation_status:
                        quotationData.quotationStatus || 'Pending',
                    status: 'Draft'
                };

                if (quotationData.dateIssued)
                    qtPayload.date_issued = quotationData.dateIssued;
                if (quotationData.orderDescription)
                    qtPayload.order_description =
                        quotationData.orderDescription.trim();
        ...
        try {
            ...
            if (projectToDelete?.quotation_id) {
                await supabase
                    .from('quotations')
                    .update(softDeletePayload)
                    .eq('id', projectToDelete.quotation_id);
            }
            ...
            if (currentUser)
                logDelete(
                    ...
                    'SALES',
                    'PROJECT',
                    projectToDelete?.project_number,
                    projectToDelete
                );
        } catch (err) {
            console.error(err);
    ...
    const getProjectWithAllDocuments = async (projectId) => {
        const [
            ...
            supabase
                .from('projects')
                .select(
                    '*, company:companies(*), contact:company_contacts(*), quotations!projects_quotation_id_fkey(*)'
                )
                .eq('id', projectId)
                .single(),
            supabase
    ...
    const deleteAcknowledgementReceipt = async (id) => {
        try {
            ...
            toast({
                title: 'Success',
                description: `AR '${ar?.ar_number}' moved to Trash`
            });
            return true;
        } catch (error) {
            ...
            toast({
                title: 'Error',
                description: 'Failed to delete Acknowledgement Receipt.',
                variant: 'destructive'
            });
            throw error;
        }
