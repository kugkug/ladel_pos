...
} from '@/lib/documentFlowUtils';

const StatusBadge = ({ status }) => {
    let color = 'bg-gray-100 text-gray-700';
    if (['Completed', 'Completed Issued', 'Paid', 'Confirmed'].includes(status))
...
const ProjectDetailsPage = () => {
    ...
    const { getProjectWithAllDocuments, updateProject } =
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (!result || !result.project) {
                toast({
                    title: 'Error',
                    description: 'Project not found or could not be loaded.',
                    variant: 'destructive'
                });
            ...
            if (type === 'Quotation') {
                ...
                await supabase
                    ...
                    .update({
                        ...
                        po_tax_type: null,
                        po_vat_amount: null,
                        po_amount_inclusive: null,
                        po_notes: null,
                        payment_terms: null,
                        customer_po_number: null,
                        temporary_po_code: null,
                        temporary_po_amount: null,
                        po_created_at: null
                    })
                ...
                await supabase
                    .from('projects')
                    .update({ delivery_status: calculateDeliveryStatus(drs) })
                    .eq('id', data.project?.id);
                ...
                const { data: invs } = await supabase
                    .from('invoices')
                    .select('invoice_issue_status')
                    .eq('project_id', data.project?.id)
                    .eq('is_deleted', false);
                await supabase
                    .from('projects')
                    .update({ invoice_status: calculateInvoiceStatus(invs) })
                    .eq('id', data.project?.id);
    ...
    const handlePromptPO = (qtData) => {
        if (
            window.confirm(
                'Quotation confirmed! Add Purchase Order now? \n\nClick OK to Add PO. Click Cancel for Later.'
            )
        ) {
            openModal('poAdd', null, {
                amount: qtData.gross_amount,
                tax_type: qtData.tax_type || 'VAT (12%)'
            });
        }
    };
    ...
    const openEditProjectInfo = () => {
        ...
        setProjectInfoDraft({
            pr_number:
                data.project.project_number || data.project.pr_number || '',
    ...
    const projectIdentifier = prNumber
        ? `PR No: ${prNumber}`
        : `Project: ${project?.project_title || 'Loading...'}`;
    ...
    return (
        <>
            <Helmet>
                <title>{projectIdentifier} - Details</title>
            </Helmet>
                            ...
                            {quotation?.id && (
                                <div className='flex gap-2'>
                                    <Button
                                        ...
                                        size='sm'
                                        onClick={() =>
                                            openModal('qtEdit', quotation)
                                        }
                                        className='bg-white shadow-sm hover:border-blue-300 h-9'
                                    >
                                        ...
                                        onClick={() =>
                                            promptDelete(
                                                ...
                                                quotation,
                                                {
                                                    ...
                                                    date: quotation?.date_issued,
                                                    amount: quotation?.gross_amount
                                                }
                                            )
                                        }
                                    >
                            ...
                            {quotation?.id ? (
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                                    ...
                                    <div className='bg-blue-50/50 p-5 rounded-xl border border-blue-100 space-y-2 text-sm'>
                                        ...
                                        <div className='flex justify-between font-bold text-xl text-blue-800 pt-3 mt-1 border-t border-blue-200'>
                                            <span>Total Amount:</span>{' '}
                                            <span>
                                                {formatCurrency(
                                                    quotation?.gross_amount
                                                )}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                                    <div>
                                        ...
                                        <p className='text-sm text-gray-500 mb-2'>
                                            ...
                                            {project?.po_date ||
                                                new Date(
                                                    project?.po_created_at ||
                                                        Date.now()
                                                )
                                                    .toISOString()
                                                    .split('T')[0]}
                                        </p>
                                        <p className='text-sm font-semibold text-gray-700 bg-gray-50 p-2 rounded inline-block mb-4'>
                                            Terms:{' '}
                                            {project?.payment_terms || 'N/A'}
                                        </p>
                        ...
                        <div className='bg-gray-50 border-b border-gray-100 px-6 py-4 flex items-center justify-between'>
                            <div className='flex items-center gap-2'>
                                ...
                                <h3 className='font-bold text-gray-900'>
                                    3. Delivery Receipts{' '}
                                    <StatusBadge
                                        status={project?.delivery_status}
                                    />
                                </h3>
                            </div>
                            <Button
                                variant='outline'
                                size='sm'
                                onClick={() => openModal('drAdd')}
                                disabled={
                                    project?.delivery_status === 'Completed'
                                }
                                className='bg-white shadow-sm hover:border-blue-300 h-9'
                            >
                            ...
                            {deliveryReceipts && deliveryReceipts.length > 0 ? (
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                    {deliveryReceipts.map((dr) => (
                                        <div
                                            key={dr.id}
                                            className='border border-gray-200 p-4 rounded-xl shadow-sm bg-gray-50/50 hover:bg-white transition-colors group'
                                        >
                                            ...
                                            <p className='text-xs text-gray-600 line-clamp-2 mb-4 h-8'>
                                                {dr?.notes || 'No notes'}
                                            </p>
                                    ...
                                    <table className='w-full text-sm text-left'>
                                        <thead className='bg-gray-50 border-b border-gray-200'>
                                            <tr>
                                                ...
                                                <th className='p-4 text-gray-600 text-right'>
                                                    Total Amount
                                                </th>
                                                <th className='p-4 text-gray-600 text-center'>
                                                    Status
                                                </th>
                                                <th className='p-4 text-gray-600 text-center'>
                                                    Action
                                        ...
                                        <tbody className='divide-y divide-gray-100'>
                                            {invoices.map((inv) => (
                                                ...
                                                >
                                                    ...
                                                    <td className='p-4 font-bold text-blue-700 text-right'>
                                                        {formatCurrency(
                                                            inv?.total_amount
                                                        )}
                                                    </td>
                        ...
                        <div className='p-6'>
                            <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-6'>
                                ...
                                <div className='bg-red-50 p-4 rounded-xl border border-red-200'>
                                    ...
                                    <p className='font-bold text-2xl text-red-800 mt-1'>
            ...
            {/* Modals */}
            <QuotationEditModal
                isOpen={modals.qtEdit}
                onClose={() => closeModal('qtEdit')}
                data={activeItem}
                onSaveSuccess={loadData}
                onPromptPO={handlePromptPO}
            />
            <QuotationPreviewModal
                isOpen={modals.qtPreview}
                onClose={() => closeModal('qtPreview')}
                data={activeItem}
            />

            <AddPOModal
                isOpen={modals.poAdd}
                onClose={() => closeModal('poAdd')}
                project={project}
                prefillData={prefillPOData}
                onSaveSuccess={loadData}
            />
            <POEditModal
                isOpen={modals.poEdit}
                onClose={() => closeModal('poEdit')}
                data={activeItem}
                onSaveSuccess={loadData}
            />
            <POPreviewModal
                isOpen={modals.poPreview}
                onClose={() => closeModal('poPreview')}
                data={activeItem}
            />

            <AddDRModal
                isOpen={modals.drAdd}
                onClose={() => closeModal('drAdd')}
                project={project}
                onSaveSuccess={loadData}
                onPromptInvoice={() => openModal('invAdd')}
            />
            <EditDRModal
                isOpen={modals.drEdit}
                onClose={() => closeModal('drEdit')}
                data={activeItem}
                project={project}
                onSaveSuccess={loadData}
            />
            <DRPreviewModal
                isOpen={modals.drPreview}
                onClose={() => closeModal('drPreview')}
                data={activeItem}
            />

            <AddInvoiceModal
                isOpen={modals.invAdd}
                onClose={() => closeModal('invAdd')}
                project={project}
                onSaveSuccess={loadData}
            />
            <EditInvoiceModal
                isOpen={modals.invEdit}
                onClose={() => closeModal('invEdit')}
                data={activeItem}
                project={project}
                onSaveSuccess={loadData}
            />
            <InvoicePreviewModal
                isOpen={modals.invPreview}
                onClose={() => closeModal('invPreview')}
                data={activeItem}
            />

            <AddPaymentModal
                isOpen={modals.payAdd}
                onClose={() => closeModal('payAdd')}
                project={project}
                invoices={invoices}
                onSaveSuccess={loadData}
            />
            <EditPaymentModal
                isOpen={modals.payEdit}
                onClose={() => closeModal('payEdit')}
                data={activeItem}
                project={project}
                invoices={invoices}
                onSaveSuccess={loadData}
            />
            <PaymentPreviewModal
                isOpen={modals.payPreview}
                onClose={() => closeModal('payPreview')}
                data={activeItem}
            />
            ...
            <Dialog
                open={isEditProjectInfoOpen}
                onOpenChange={(open) =>
                    !open && setIsEditProjectInfoOpen(false)
                }
            >
                <DialogContent className='max-w-lg bg-white'>
                    ...
                    <form
                        onSubmit={submitProjectInfoEdit}
                        className='space-y-4'
                    >
                        ...
                        <DialogFooter className='pt-2'>
                            <Button
                                type='button'
                                variant='outline'
                                onClick={() => setIsEditProjectInfoOpen(false)}
                            >
                                Cancel
                            </Button>
                            <Button
