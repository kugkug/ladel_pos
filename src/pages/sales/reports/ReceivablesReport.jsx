...
import { formatPHP, generateMonths, getMonthKey } from '@/lib/reportUtils';
import { format } from 'date-fns';
import { Download } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { exportToCSV } from '@/lib/csvExport';
import { getUpcomingTerms, getOverdueTerms } from '@/lib/dashboardDataUtils';
...
const ReceivablesReport = () => {
      ...
      try {
        const { data: invoices, error } = await supabase
          ...
          .select(`
            ...
            projects (
              id, project_number, po_number, project_title,
        ...
        const upcoming = getUpcomingTerms(invoices);
        const overdue = getOverdueTerms(invoices);
        
        const uniqueTermsMap = new Map();
        [...overdue, ...upcoming].forEach(t => uniqueTermsMap.set(t.id, t));
        const combined = Array.from(uniqueTermsMap.values());

        const formatted = combined.map(term => {
          const inv = invoices.find(i => i.id === term.id);
          const proj = inv?.projects || {};
          const monthKey = getMonthKey(term.dueDate);

          return {
            id: term.id,
            companyName: proj.companies?.company_name || 'Unknown',
            prNumber: proj.project_number || '-',
            poNumber: proj.po_number || '-',
            projectTitle: proj.project_title || 'Unknown Project',
            dueDate: term.dueDate ? format(new Date(term.dueDate), 'MM/dd/yyyy') : '-',
            amount: term.amount,
            monthKey: monthKey
          };
        });
  ...
  const handleDownloadCSV = () => {
    const headers = [
      'Company Name', 'PR Number', 'PO Number', 'Project Title', 'Due Date',
    ...
    exportToCSV(csvData, `Receivables_${startLabel}_${endLabel}.csv`, headers);
  };
        ...
        <table className="reports-table">
          <thead>
            <tr>
              ...
              <th className="sticky-header min-w-[100px]">Due Date</th>
              {filteredMonths.map(m => (
                <th key={m.key} className="sticky-header min-w-[140px] text-right">{m.label}</th>
              ))}
              <th className="sticky-header min-w-[150px] text-right font-bold text-primary">Grand Total</th>
            </tr>
          </thead>
          <tbody>
                ...
                <React.Fragment key={company}>
                  ...
                  {groupedData[company].map(row => (
                    <tr key={row.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td>{row.poNumber}</td>
                      <td className="truncate max-w-[250px]" title={row.projectTitle}>{row.projectTitle}</td>
                      <td>{row.dueDate}</td>
                      {filteredMonths.map(m => (
