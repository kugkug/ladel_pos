...
const ProjectInProgressReport = () => {
    ...
    const fetchData = async () => {
      try {
        const { data: projects, error } = await supabase
          ...
          .in('status', ['Ongoing', 'ongoing', 'In Progress', 'Active'])
          .eq('is_deleted', false);

        if (error) throw error;

        const formatted = projects.map(p => {
          const poDate = p.po_created_at || p.created_at;
          const amount = parseAmount(p.po_amount_inclusive || p.temporary_po_amount);
  ...
  const handleDownloadCSV = () => {
    const headers = [
      'Company Name', 'PR Number', 'PO Number', 'Project Title',
      ...filteredMonths.map(m => `Amt PO ${m.label}`),
      'Grand Total'
    ];
    ...
    exportToCSV(csvData, `ProjectInProgress_${startLabel}_${endLabel}.csv`, headers);
  };
          ...
          <thead>
            <tr>
              ...
              {filteredMonths.map(m => (
                <th key={m.key} className="sticky-header min-w-[140px] text-right">Amt PO {m.label}</th>
              ))}
                ...
                <React.Fragment key={company}>
                  ...
                  {groupedData[company].map(row => (
                    <tr key={row.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      {filteredMonths.map(m => (
                        <td key={m.key} className="text-right tabular-nums text-amber-600 font-medium">
          ...
          {displayData.length > 0 && (
            <tfoot>
              <tr className="report-footer">
                ...
                {filteredMonths.map(m => (
                  <td key={m.key} className="text-right text-amber-700">{formatPHP(totals[m.key])}</td>
                ))}
...
export default ProjectInProgressReport;